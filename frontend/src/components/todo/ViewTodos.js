...
function Todos({isAuthenticated, setIsAuthenticated}) {
	...
	const markCompleted = async (id) => {
		try {
      await axios.put(`http://poda-polina-blue-green-lb-1504784492.us-east-1.elb.amazonaws.com/api/todo${id}/markcomplete`, {}, {
				headers: {
	...
	const deleteTodo = async (id) => {
		try {
      await axios.delete(`http://poda-polina-blue-green-lb-1504784492.us-east-1.elb.amazonaws.com/api/todo${id}`, {
				headers: {
					'Authorization': `Bearer ${sessionStorage.getItem('token')}`
				}
			});
